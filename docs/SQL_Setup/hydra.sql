...
CREATE TABLE `hydra_jobboard` (
  ...
  `failedNodes` varchar(150) NOT NULL DEFAULT '' COMMENT 'A list of nodes that the job has failed on, each node delimited by a single space. ',
  ...
  `frameDirectory` varchar(256) DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `hydra_taskboard` (
  ...
  `host` varchar(80) NOT NULL COMMENT 'Host the task is running on',
