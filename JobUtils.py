...
def prioritizeJob(job_id, priority):
    with transaction() as t:
        t.cur.execute("UPDATE hydra_jobboard SET priority = '%d' WHERE id = '%d'" % (priority, job_id))
        t.cur.execute("UPDATE hydra_taskboard SET priority = '%d' WHERE job_id = '%d'" % (priority, job_id))
